            ...
            <div key={idx} className="grid grid-cols-1 gap-1.5 pt-0.5">
              {block.items.map((item, bIdx) => {
                ...
                return (
                  <div
                    key={bIdx}
                    className={`p-2.5 rounded-xl border flex items-center justify-between gap-2.5 text-[11px] sm:text-xs font-medium transition-all shadow-2xs ${
                      isMet
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="truncate leading-snug">{renderFormattedInlineText(cleanItem.replace(/—\s*(✓|✗).*$/, ''))}</span>
                    </div>
